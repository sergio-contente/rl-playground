...
class Agent:
	...
	def __init__(self):
		self.n_games = 0
		self.epsilon = 0 # e-greedy parameter
	...
	def train_long_memory(self):
		if len(self.memory) > BATCH_SIZE:
			mini_sample = random.sample(self.memory, BATCH_SIZE) #list of memory tuples
		else:
	...
	def get_action(self, state):
		...
		final_move = [0,0,0]
		if random.randint(0, 200) < self.epsilon: #explore -> random move
...
def train():
	...
	while True:
		...
		if done:
			# train longe memory -> Experience Replay
			# plot results
			game.reset()
			agent.n_games += 1
